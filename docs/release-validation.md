...
| Update path gate | Reproducible launcher-driven update proof from installed `RC18` to `RC19` | Manual checklist below | No |
...
- the RC number matches the patch number: `RC18 -> 0.1.18`, `RC19 -> 0.1.19`
...
pwsh -Command "cargo wix --package mfa-forge-gui --no-build --target-bin-dir target/release --output target/rc/MFA-Forge-RC19-x64.msi"
...
## Update path checklist for RC19

This checklist must pass before any RC19 publication decision:

- local baseline is the installed `RC18` MSI
- launcher can discover the intended RC19 GitHub prerelease metadata and asset names
- launcher downloads the RC19 MSI and validates its SHA256 against the published checksum
- launcher hands control to the MSI update flow without stealth or background behavior
- MSI upgrade replaces the installed RC18 binaries with RC19 while preserving the per-user install boundary
...
- update-path findings for `RC18 -> RC19`
